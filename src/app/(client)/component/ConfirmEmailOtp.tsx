...
export default function ConfirmEmailOtp() {
  ...
  const { otp, valid, error, handleChange, validate } = useOtpValidator(6);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire(
        "Thành công",
        res.message || "Xác minh email thành công!",
        "success"
      );
    ...
    <div className="otp-wrapper">
      <div className="otp-box">
        ...
        <p className="otp-desc">
          Nhập mã OTP đã gửi tới email <strong>{email}</strong> để hoàn tất đăng
          ký.
        </p>
          ...
          <div className="otp-code-group">
            {otp.map((digit, i) => (
              <input
                ...
                className={`otp-code ${valid[i] ? "otp-valid" : "otp-invalid"}`}
              />
            ))}
          </div>
